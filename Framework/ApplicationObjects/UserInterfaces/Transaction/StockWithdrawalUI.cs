...
namespace NSites.ApplicationObjects.UserInterfaces.Transaction
{
    public partial class StockWithdrawalUI : Form
    {
        ...
        */
            ...
            try
            {
                if (dgvList.Rows.Count > 0)
                {
                    foreach (DataRow _drF in loInventoryHeader.getAllData("", dgvList.CurrentRow.Cells[0].Value.ToString(), "", "Stock Withdrawal").Rows)
                    {
                        if (_drF["Final"].ToString() == "N")
                        {
                            MessageBoxUI mb = new MessageBoxUI("You can only CANCEL finalized Stock Withdrawal!", GlobalVariables.Icons.Information, GlobalVariables.Buttons.OK);
                            mb.ShowDialog();
                            return;
                        }
                        else if (_drF["Cancel"].ToString() == "Y")
                        {
                            MessageBoxUI mb = new MessageBoxUI("Stock Withdrawal is already CANCELLED!", GlobalVariables.Icons.Information, GlobalVariables.Buttons.OK);
                            mb.ShowDialog();
                            return;
                        }
                    }
        ...
        private void StockWithdrawalUI_KeyUp(object sender, KeyEventArgs e)
        {
